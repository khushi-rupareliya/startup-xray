...
function buildDynamicReport(inputs, mlScore) {
  ...
  // ── key scores (0-10, derived from inputs) ──
  const ideaScore = usp === "Yes" ? (businessModel === "Clear" ? 8 : 6.5) : 5;
  const marketScore = marketSize === "Large" ? 9 : marketSize === "Medium" ? 7 : 5;
  const bizScore = revenueModel === "Yes" ? (customerValidation === "Paying customers" ? 8.5 : customerValidation === "Early users" ? 7 : 6) : 4.5;
  ...
  const label = (n) => (n >= 8 ? "Strong" : n >= 6.5 ? "Moderate" : "Weak");

  // ── insights & risks (dynamic) ──
  const insights = [];
  const risks = [];

  if (marketSize === "Large" || marketGrowth === "High") insights.push(`${marketGrowth} growth rate in the ${industryLabel} market supports scalability`);
  if (usp === "Yes") insights.push("Defined USP positions the startup for differentiation");
  if (customerValidation === "Paying customers") insights.push("Paying customers validate product-market fit");
  else if (customerValidation === "Early users") insights.push("Early user traction shows initial demand");
  if (teamSkills === "Strong" || teamSkills === "Balanced") insights.push("Team skill coverage supports strong execution");
  if (founderExperience === "Experienced") insights.push("Experienced founders reduce early-stage execution risk");
  if (insights.length < 2) insights.push(`${stageLabel} stage startups in ${industryLabel} have strong funding opportunities`);

  if (competition === "High") risks.push("Highly competitive market requires strong differentiation strategy");
  if (fundingStage === "Bootstrapped") risks.push("Limited funding may slow growth and hiring pace");
  if (customerValidation === "No users") risks.push("No customer validation yet — product-market fit is unproven");
  if (teamSkills === "Limited") risks.push("Limited team skill coverage may create execution bottlenecks");
  if (founderExperience === "First-time") risks.push("First-time founders face a steeper learning curve");
  if (risks.length < 2) risks.push("Market conditions and investor appetite can shift rapidly");
  ...
  const recKey = pct >= 70 ? "high" : pct >= 45 ? "medium" : "low";
  ...
  return {
    ...
    scores: {
      ideaStrength:        { score: ideaScore,  label: label(ideaScore)  },
      marketPotential:     { score: marketScore, label: label(marketScore) },
      businessViability:   { score: bizScore,   label: label(bizScore)   },
      executionCapability: { score: execScore,  label: label(execScore)  },
      financialStrength:   { score: finScore,   label: label(finScore)   },
    },
    insights: insights.slice(0, 3),
    risks:    risks.slice(0, 3),
    recommendation: recMap[recKey],
  };
}
...
function App() {
  ...
  const [mlScore, setMlScore]   = useState(null);   // ← actual score from backend
  const [report, setReport]     = useState(null);   // ← dynamic report
  ...
  // ── Step 1: analyze idea via backend ────────────────────────────────────────
  ...
  // ── Step 5 → 6: get ML prediction then build dynamic report ─────────────────
  const handleSubmitAndGoToResults = async () => {
    ...
    try {
      const res = await fetch("https://startup-xray.onrender.com/predict", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          // send ALL collected data so backend can use it
          idea, industry,
      ...
      const score = probability;   // 0–1 float from backend
      setMlScore(score);

      // build the report dynamically from actual inputs + real ML score
      const inputs = {
      ...
      setReport(buildDynamicReport(inputs, score));
      setStep(6);
    } catch (err) {
      ...
      <div className="top-bar">
        ...
        <div className="steps">
          {["Describe Idea","Startup Basics","Business Strength","Execution Power","Financial & Market Signals","Result Dashboard"]
            .map((label, i) => (
              <span key={i} className={step === i + 1 ? "active" : ""}>{label}</span>
            ))}
        </div>
      </div>
      ...
      <div className="content">
        ...
        {step === 1 && (
          <div className="step-container">
            <div className="left card">
              ...
              <textarea
                style={{ fontFamily: "inherit" }}   /* ← fixes monospace bug */
                value={idea}
        ...
        {step === 5 && (
          <div className="panel card">
            ...
            <div className="buttons">
              ...
              {/* ← THIS is where handleSubmit is called — was missing before */}
              <button
                onClick={handleSubmitAndGoToResults}
        ...
        {step === 6 && report && (
          <div className="panel">
            ...
            {/* Overview — 4 cards instead of text wall */}
            <div className="form-section">
